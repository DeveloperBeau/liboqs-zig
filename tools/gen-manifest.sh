#!/usr/bin/env bash
# Regenerate build/manifest.zig, include/oqs/oqsconfig.h, and src/algorithms.zig
...
cfg.append("// Portable-only build configuration for liboqs-zig, scoped to the families that")
...
# -------------------------------------------------------------------------
# Typed-API registry: src/algorithms.zig. One typed wrapper per concrete
# enabled variant. Concrete ids + sizes come from per-family *_length_*
# defines; name strings from kem.h/sig.h OQS_*_alg_<id> macros. Scoped to
# build/enabled-families.txt (same source of truth as oqsconfig.h above).
# -------------------------------------------------------------------------
KEM_NAMES = alg_name_table(os.path.join(SRC, "kem", "kem.h"), "OQS_KEM_alg_")
SIG_NAMES = alg_name_table(os.path.join(SRC, "sig", "sig.h"), "OQS_SIG_alg_")


def type_name(variant_id):
    """ml_kem_768 -> MlKem768 (capitalize first char of each _-segment)."""
    return "".join(seg[:1].upper() + seg[1:] for seg in variant_id.split("_"))


def collect_kem_registry():
    out = []
    for fam in enabled_families:
        hdr = os.path.join(SRC, "kem", fam, "kem_%s.h" % fam)
        if not os.path.exists(hdr):
            continue
        text = read(hdr)
        ids = re.findall(r"#define\s+OQS_KEM_([A-Za-z0-9_]+)_length_public_key\s+(\d+)", text)
        for vid, pk in ids:
            def length(suffix):
                m = re.search(r"#define\s+OQS_KEM_%s_length_%s\s+(\d+)" % (re.escape(vid), suffix), text)
                if not m:
                    raise SystemExit("missing OQS_KEM_%s_length_%s in %s" % (vid, suffix, hdr))
                return int(m.group(1))
            if vid not in KEM_NAMES:
                warnings.append("kem id %s has no OQS_KEM_alg_ name macro; skipped" % vid)
                continue
            out.append((vid, KEM_NAMES[vid], fam, int(pk),
                        length("secret_key"), length("ciphertext"), length("shared_secret")))
    out.sort(key=lambda e: (e[2], e[0]))
    return out


def collect_sig_registry():
    out = []
    for fam in enabled_families:
        hdr = os.path.join(SRC, "sig", fam, "sig_%s.h" % fam)
        if not os.path.exists(hdr):
            continue
        text = read(hdr)
        ids = re.findall(r"#define\s+OQS_SIG_([A-Za-z0-9_]+)_length_public_key\s+(\d+)", text)
        for vid, pk in ids:
            def length(suffix):
                m = re.search(r"#define\s+OQS_SIG_%s_length_%s\s+(\d+)" % (re.escape(vid), suffix), text)
                if not m:
                    raise SystemExit("missing OQS_SIG_%s_length_%s in %s" % (vid, suffix, hdr))
                return int(m.group(1))
            if vid not in SIG_NAMES:
                warnings.append("sig id %s has no OQS_SIG_alg_ name macro; skipped" % vid)
                continue
            out.append((vid, SIG_NAMES[vid], fam, int(pk),
                        length("secret_key"), length("signature")))
    out.sort(key=lambda e: (e[2], e[0]))
    return out


kem_reg = collect_kem_registry()
sig_reg = collect_sig_registry()

reg = []
reg.append("// GENERATED by tools/gen-manifest.sh from liboqs %s headers. Do not edit by hand." % VERSION)
reg.append("// Typed wrappers, one per concrete enabled algorithm. Type names are the")
reg.append("// variant id with each _-segment's first char capitalized (ml_kem_768 -> MlKem768).")
reg.append("const typed = @import(\"typed.zig\");")
reg.append("")
reg.append("/// Key encapsulation mechanisms.")
reg.append("pub const kem = struct {")
for vid, name, fam, pk, sk, ct, ss in kem_reg:
    reg.append("    pub const %s = typed.Kem(.{ .name = %s, .public_key = %d, .secret_key = %d, .ciphertext = %d, .shared_secret = %d });"
               % (type_name(vid), zstr(name), pk, sk, ct, ss))
reg.append("};")
reg.append("")
reg.append("/// Digital signature schemes.")
reg.append("pub const sig = struct {")
for vid, name, fam, pk, sk, sig_max in sig_reg:
    reg.append("    pub const %s = typed.Sig(.{ .name = %s, .public_key = %d, .secret_key = %d, .signature_max = %d });"
               % (type_name(vid), zstr(name), pk, sk, sig_max))
reg.append("};")
reg.append("")

with open(os.path.join(ROOT, "src", "algorithms.zig"), "w") as f:
    f.write("\n".join(reg))
...
print("algorithms.zig: %d KEM wrappers, %d SIG wrappers" % (len(kem_reg), len(sig_reg)), file=sys.stderr)
if warnings:
...
echo "Done. Wrote build/manifest.zig, include/oqs/oqsconfig.h, and src/algorithms.zig" >&2
