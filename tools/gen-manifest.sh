...
# -------------------------------------------------------------------------
# Maximal oqsconfig.h. Enable list is derived from the C-preprocessor guards
...
cfg.append("// Maximal portable-only build configuration for ZigOQS:")
cfg.append("// every KEM/SIG family + portable variant enabled; UOV (needs OpenSSL),")
cfg.append("// stateful signatures, and all optimized backends are excluded.")
cfg.append("")
...
for m in sig_macros:
    cfg.append("#define %s 1" % m)
cfg.append("")
cfg.append("// UOV (requires OpenSSL), stateful signatures (XMSS/LMS), and all")
cfg.append("// optimized backends are intentionally left undefined.")
cfg.append("")
